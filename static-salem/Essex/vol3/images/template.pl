...
open(FILE, "/web/data/salem/witchcraft/Essex/vol3/gifs/name") || die "Couldn't open file for reading\n";
while(<FILE>) {
   push(@files,"$_");
}
close(FILE);
...
for($i=$index; $i<$cnt; $i++) {
   ...
   open(OUT, "> /web/data/salem/witchcraft/Essex/vol3/images/$file.html") || die "Cannot open $file.html for writing\n";
