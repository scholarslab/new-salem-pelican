...
open(FILE, "/web/data/salem/witchcraft/Perley/vol2/jpgs/name") || die "Couldn't open file for reading\n";
while(<FILE>) {
   push(@files,"$_");
}
close(FILE);
...
for($i=0; $i<$cnt; $i++) {
   ...
   $file=$jpg;
   $file=~s/\.jpg$//;
   open(OUT, "> /web/data/salem/witchcraft/Perley/vol2/images/$file.html") || die "Cannot open $file.html for writing\n";
