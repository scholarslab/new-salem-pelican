...
open(FILE, "/web/data/salem/witchcraft/Essex/vol7/gifs/name") || die "Couldn't open file for reading\n";
while(<FILE>) {
   push(@files,"$_");
}
close(FILE);
...
for($i=0; $i<$cnt; $i++) {
   ...
   open(OUT, "> /web/data/salem/witchcraft/Essex/vol7/images/$file.html") || die "Cannot open $file.html for writing\n";
